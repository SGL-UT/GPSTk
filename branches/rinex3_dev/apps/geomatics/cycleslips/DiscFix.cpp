...
int main(int argc, char **argv)
{
   try
   {
      ...
      Title += PrgmEpoch.asString();
      cout << Title;

      config.SVonly.setfill('0'); // set fill char in GSatID
...
//   1 couldn't open file, 2 file doesn't have required data.
...
int ProcessOneSatOneEpoch(GSatID sat, CommonTime tt, unsigned short& flag,
                          vector<double>& data,
                          vector<unsigned short>& lli, vector<unsigned short>& ssi)
   throw(Exception)
{
   try
   {
...
// This will write out passes only if ProcessSatPass() has been called. It
...
void WriteRINEXheader(void) throw(Exception)
{
   try
   {
      ...
      rheadout.date = PrgmEpoch.asString();
      ...
      rheadout.firstObs = config.FirstEpoch;
      rheadout.valid |= RinexObsHeader::firstTimeValid;
      rheadout.interval = config.dt;
...
void PrintSPList(ostream& os, string msg, vector<SatPass>& v, bool printTime)
{
   ...
   os << "#" << leftJustify(msg,4)
      << " gap tot sat  ok  s      start time        end time  dt" << endl;
...
int GetCommandLine(int argc, char **argv) throw(Exception)
{
 try
 {
   ...
   if (Par.hasErrors())
   {
      if (!help && !DChelp)
      {
         cout << endl << "Errors found in command line input:" << endl;
         Par.dumpErrors(cout);
         cout << "...end of Errors.  For help run with option --help" << endl << endl;
      }
      help = true;
   }
   ...
   //if (config.verbose)
   //{ // if GDCorrector::Debug is not set higher, set to 2
   ...
   if (config.begTime > CommonTime::BEGINNING_OF_TIME)
      config.oflog << " Begin time is " << config.begTime.asString() << endl;
   if (config.endTime < CommonTime::END_OF_TIME)
      config.oflog << " End time is " << config.endTime.asString() << endl;
   if (config.UseCA)
      config.oflog << " 'Use the C/A pseudorange' flag is set" << endl;
   else
      config.oflog << " Do not use C/A code range (C1) unless P1 is absent" << endl;
   ...
   if (config.ExSV.size())
   {
      ...
      {
         if (config.ExSV[i].id == -1)
            config.oflog << " (all " << config.ExSV[i].systemString() << ")" << endl;
         else
            config.oflog << " " << config.ExSV[i];
      }
      config.oflog << endl;
   }
