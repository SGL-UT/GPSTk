...
namespace gpstk
{
   //--------------------------------------------------------------------------
   //--------------------------------------------------------------------------
   Xvt GalEphemerisStore::getXvt(const SatID sat, const DayTime& t)
      const throw(InvalidRequest)
   {
      short ref;
      return getXvt(sat, t, ref);
   } // end of GalEphemerisStore::getXvt()


   //--------------------------------------------------------------------------
   //--------------------------------------------------------------------------
   Xvt GalEphemerisStore::getXvt(const SatID sat, const DayTime& t, short& ref)
      const throw(InvalidRequest)
   {
      try
      {
   ...
   //--------------------------------------------------------------------------
   const EngEphemeris&
   GalEphemerisStore::findEphemeris(const SatID sat, const DayTime& t) 
      const throw(InvalidRequest)
   {
      try
      {
   ...
   //--------------------------------------------------------------------------
   //--------------------------------------------------------------------------
   short GalEphemerisStore::getSatHealth(const SatID sat, const DayTime& t)
      const throw(InvalidRequest)
   {
      try
      {
   ...
   } // end of GalEphemerisStore::getHealth()
   ...
   //--------------------------------------------------------------------------
   bool GalEphemerisStore::addEphemeris(const EngEphemeris& eph)
      throw()
   {
      bool rc = false;
      DayTime t(0.L);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
   void GalEphemerisStore::edit(const DayTime& tmin, const DayTime& tmax)
      throw()
   {
      DayTime test;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
   unsigned GalEphemerisStore::ubeSize() const throw()
   {
      unsigned counter = 0;
...
//-----------------------------------------------------------------------------
   const EngEphemeris&
   GalEphemerisStore::findUserEphemeris(const SatID sat, const DayTime& t) 
      const throw(InvalidRequest)
   {
      DayTime test;
      ...
      return it->second;
   } // end of GalEphemerisStore::findEphemeris()


//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
   const EngEphemeris&
   GalEphemerisStore::findNearEphemeris(const SatID sat, const DayTime& t) 
      const throw(InvalidRequest)
   {
      DayTime test;
      UBEMap::const_iterator prn_i = ube.find(sat.id);
      ...
      return it->second;
   } // end of GalEphemerisStore::findNearEphemeris()


//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
   int GalEphemerisStore::addToList(std::list<EngEphemeris>& v) const throw()
   {
      int n=0;
      ...
      return n;
   } // end of GalEphemerisStore::addToList(list<EngEphemeris>&)

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//   const EngEphMap 
   const std::map<DayTime, EngEphemeris>& 
   GalEphemerisStore::getEphMap( const SatID sat )
            const throw(InvalidRequest)
   {
      UBEMap::const_iterator prn_i = ube.find(sat.id);
      if (prn_i == ube.end())
      ...
      return(prn_i->second);
   } // end of GalEphemerisStore::getEphMap(const SatID sat)
