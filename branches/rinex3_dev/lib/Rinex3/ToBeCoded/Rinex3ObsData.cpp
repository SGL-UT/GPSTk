...
namespace gpstk
{
      ...
      *this=rod;
      ...
      if(epochFlag==0 || epochFlag==1 || epochFlag==6) {
         ...
         for (isv=1, ndx=0; ndx<numSvs; isv++, ndx++) {
            ...
            try {
               satIndex[ndx] = Rinex3SatID(line.substr(col+isv*3-1, 3));
            }
            catch (Exception& e)
            { 
         ...
         for (isv=0; isv < numSvs; isv++)
         {
            ...
            for (ndx=0, line_ndx=0; ndx < numObs; ndx++, line_ndx++)
            {
               ...
               Rinex3ObsHeader::ObsID obs_type = hdr.obsTypeList[ndx];
               if (! (line_ndx % 5))
               {
                  strm.formattedGetLine(line);
                  line_ndx = 0;
      ...
      throw(FFStreamError)
   {
      try
      {
         ...
         // Real Rinex3 has epochs 'yy mm dd hr 59 60.0' surprisingly often....
         double ds=0;
         if(sec >= 60.) { ds=sec; sec=0.0; }
         CommonTime rv(yy+year, month, day, hour, min, sec);
         if(ds != 0) rv += ds;
   ...
   string Rinex3ObsData::writeTime(const CommonTime& dt) const
   ...
   void Rinex3ObsData::dump(ostream& s) const
   {
      ...
      if(epochFlag == 0 || epochFlag == 1) 
      {
         ...
         for(it=obs.begin(); it!=obs.end(); it++) {
            s << "Sat " << setw(2) << Rinex3SatID(it->first);
