...
namespace gpstk
{
   ...
   void Rinex3ClockHeader::clear(void)
   {
      ...
      clkNameList.clear();
      staCoordList.clear();
      timeFirst = DayTime::BEGINNING_OF_TIME;
      ...
      throw(FFStreamError)
   {
      ...
      else if (label == numRefClkString)
      {
         ...
         {
            DayTime T0 = parseTime( line.substr(7,26) );
               // only one time
            if( timeFirst == DayTime::BEGINNING_OF_TIME )
            {
               timeFirst = T0;
            }
               // left boundary
       ...
       */
   DayTime Rinex3ClockHeader::parseTime(const string& line) const
   {
   ...
   string Rinex3ClockHeader::writeTime(const DayTime& dt) const
   {
   ...
   }  // End of method 'Rinex3ClockHeader::writeTime(const DayTime& dt)'
