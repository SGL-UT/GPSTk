...
#ifndef GPSTK_RINEX3_SATID_HPP
#define GPSTK_RINEX3_SATID_HPP
...
/**
 * @file Rinex3SatID.hpp
 * gpstk::Rinex3SatID - navigation system-independent representation of a satellite
 * as defined by the RINEX3 specification.
 */

namespace gpstk
{
   class Rinex3SatID : public SatID
   {
   public:

      /// empty constructor, creates an invalid object
      Rinex3SatID() throw() { id=-1; system=systemGPS; }

      /// explicit constructor, no defaults, RINEX3 systems only
      Rinex3SatID(int p, SatelliteSystem s) throw()
      {
         id = p; system = s;
         switch(s) {
            case systemGPS:
            ...
            // invalidate anything non-RINEX3
            default:
      ...
      Rinex3SatID(const std::string& str) throw(Exception)
      {
         try { fromString(str); }
         catch(Exception& e) { GPSTK_RETHROW(e); }
      }

      /// cast SatID to Rinex3SatID
      Rinex3SatID(const SatID& sat) throw()
      { *this = Rinex3SatID(sat.id,sat.system); }
      ...
      /// @note return only RINEX3 types, for non-RINEX3 systems return '?'
      ...
      void fromString(const std::string s) throw(Exception)
      {
         ...
         switch(c)
         {
            ...
            case ' ': case 'G': case 'g':
               system = SatID::systemGPS;
               break;
            default:                   // non-RINEX3 system character
   ...
   /// stream output for Rinex3SatID
   inline std::ostream& operator<<(std::ostream& s, const Rinex3SatID& sat)
   {
      s << sat.toString();
      return s;
   }
