...
namespace gpstk
{
  ...
  public:
    ...
    typedef std::map<ObsID, RinexDatum> ObsIDMap;
    /// map from SatID to ObsIDMap
    typedef std::map<RinexSatID, ObsIDMap> Rinex3SatMap;
  ...
  protected:
     ...
     * yourself.  This means making sure that "COMMENT" is at the end of the
     * line and that they're the correct length (<= 80 chrs).
     * Also, make sure to correctly set the epochFlag to the correct number
     * for the type of header data you want to write.
     */
     ...
     * If there is an error in reading from the stream, it is reset to its original
     * position and its fail-bit is set.
     * Because of the Rinex 3 Obs format, a Rinex3ObsData record returned might not
     * have data.  Check the Rinex3SatMap for empty() before using any data in it.
     * @throws StringException when a StringUtils function fails
     * @throws FFStreamError when exceptions(failbit) is set and a read or
     * formatting error occurs.  Also resets the stream to its pre-read position.
     */
  ...
  private:
    ...
    /// Writes the daytime object into RINEX 3 format.
