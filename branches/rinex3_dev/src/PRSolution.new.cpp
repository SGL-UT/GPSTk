#pragma ident "$Id: PRSolution.cpp 1854 2009-04-10 16:03:21Z raindave $"
...
// ----------------------------------------------------------------------------
// CombinationsNew.hpp
// Find all the CombinationsNew of n things taken k at a time.

#include "Exception.hpp"
#include "ECEF.hpp"

/// Class CombinationsNew will compute C(n,k), all the CombinationsNew of n things
...
class CombinationsNew {
public:
      /// Default constructor
   CombinationsNew(void)
      throw()
   {
      nc = n = k = 0;
      Index = NULL;
      ...
      /// Constructor for C(n,k) = CombinationsNew of n things taken k at a time (k <= n)
      /// @throw on invalid input (k>n).
   CombinationsNew(int N, int K)
      throw(gpstk::Exception)
   {
      ...
      /// copy constructor
   CombinationsNew(const CombinationsNew& right)
      throw()
   {
      *this = right;
   }
   ...
   ~CombinationsNew(void)
   {
      ...
      /// Assignment operator.
   CombinationsNew& operator=(const CombinationsNew& right)
      throw()
   {
      this->~CombinationsNew();
      ...
      /// Compute the next combination, returning the number of CombinationsNew computed
      /// so far; if there are no more CombinationsNew, return -1.
   int Next(void) throw();
...
private:
   ...
   int nc;         ///< number of CombinationsNew computed so far
   int k,n;        ///< CombinationsNew of n things taken k at a time, given at c'tor
   int* Index;     ///< Index[j] = index of jth selection (j=0...k-1; I[j]=0...n-1)
};

// ----------------------------------------------------------------------------
int CombinationsNew::Next(void)
   throw()
{
   if(k < 1)
      return -1;
...
int CombinationsNew::Increment(int j)
   throw()
{
...
void CombinationsNew::init(int N, int K)
   throw(gpstk::Exception)
{
   if(K > N || N < 0 || K < 0)
   {
      gpstk::Exception e("CombinationsNew(n,k) must have k <= n, with n,k >= 0");
      GPSTK_THROW(e);
   }
...
namespace gpstk
{
      ...
      throw(Exception)
   {
      try
      {
         ...
         if(Solution.size() != 4)
         {
            Solution.resize(4); Solution = 0.0;
         }
         ...
         //}
         if(i)
            return i;  // return is 0(ok) or -4(no ephemeris)
         ...
         do{
            // compute all the CombinationsNew of N satellites taken stage at a time
            CombinationsNew Combo(N,stage);
            ...
            do{
               ...
               // get the next CombinationsNew and repeat
            } while(Combo.Next() != -1);
...
///-------------------------------------------------------------------------///
      ///TROY:: Build in the SatID's to the SVP Matrix.
      ///Since the SVP Matrix is used in the AutonomousPRSolution method,
      ///This gives us a way to figure out the SatID there.
      ...
      throw()
   {
      ...
      if(N <= 0)
         return 0;
      //SVP = Matrix<double>(N,4);   ///TROY::(N,5)?
      SVP = Matrix<double>(N,5);
      SVP = 0.0;
      ...
      for(nsvs=0,i=0; i<N; i++)
      {
         ...
         //Add the system to the 5th coll
         if(Satellite[i].system == SatID::systemGlonass)
            SVP(i,4) = 1;
         else
            SVP(i,4) = 0;
...
///-------------------------------------------------------------------------///
         ...
         throw(Exception)
   {
      ...
      try
      {
         ...
         //Vector<double> CRange(N),dX(4);   ///TROY::What about this?
         Vector<double> CRange(N),dX(5);
         //Matrix<double> P(N,4),PT,G(4,N),PG(N,N);   ///TROY::change 4's to 5's?
         Matrix<double> P(N,5),PT,G(5,N),PG(N,N);
         Xvt SV,RX;
         
         //Sol.resize(4);   ///TROY::What about here?
         Sol.resize(5);
         //Cov.resize(4,4);   ///TROY::And here?
         Cov.resize(5,5);
         Resid.resize(N);
         Slope.resize(Use.size());
         
            // define for algebraic solution
         //Vector<double> U(4),Q(N);   ///TROY::And here?
         Vector<double> U(5),Q(N);
         Matrix<double> A(P);
         ...
         do {
            ...
            for(n=0,i=0; i<Use.size(); i++)
            {
               ...
               P(n,3) = 1.0;
               P(n,4) = SVP(n,4);
               ///TROY:: P(n,4) = 1 if GLONASS, 0 if GPS
               ...
               if(Algebraic)
               {
                  ...
                  U(2) = A(n,2) = svxyz[2];
                  U(3) = A(n,3) = CRange(n);
                  ///TROY:: U(4) = P(4)??? What are all of these Matricies for?
                  U(4) = A(n,4) = P(n,4);
