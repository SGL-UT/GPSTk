...
public:
   ...
   void testLastXmitExpectingFail(const PassFailData& pfd,
                                  OrbAlmStore& oas,
                                  TestUtil& testFramework);
      /// Tests that computeXvt reports everything healthy.
   void testComputeXvt(const PassFailData& pfd,
                       OrbAlmStore& oas,
                       TestUtil& testFramework);
      /// Tests some non-healthy cases
   void testComputeXvt(OrbAlmStore& oas,
                       TestUtil& testFramework);
      /// Tests that getSVHealth reports everything healthy.
   void testGetSVHealth(const PassFailData& pfd,
                        OrbAlmStore& oas,
                        TestUtil& testFramework);
      /// Tests some non-healthy cases
   void testGetSVHealth(OrbAlmStore& oas,
                        TestUtil& testFramework);
      /// Test unhealthy satellites for GPS LNav.
   unsigned testUnhealthyLNav();
   ...
   int debugLevel;
      ...
      // List of PassFailData objects that contain definitions for
      // sepcific find( ) tests.
...
createAndDump()
{
   ...
   for (cit=dataList.begin();cit!=dataList.end();cit++)
   {
      try
      {
         ...
         if (retval & OrbAlmStore::ADD_XMIT)
         {
            addXmit++;
            addSuccess++;
         }
         if (retval & OrbAlmStore::ADD_SUBJ)
         {
            addSubj++;
            addSuccess++;
         }
   ...
   {
      stringstream ss;
      ss << "Size of ObsSysStore(subject SVs) incorrect after loading."
         << "  Expected "  << sizeSubj << " actual size " << countSubj;
      TUFAIL(ss.str());
      passed = false;
   }
   else
...
//--- Test the getXXXTime( ) methods -------------------------
   ...
   for (citp=pfList.begin();citp!=pfList.end();citp++)
   {
      ...
      //--- Test the computeXvt( ) method --------------------------------
   currMethod = typeDesc + " OrbAlmStore.computeXvt() ";
   TUCSM(currMethod);

   for (citp=pfList.begin();citp!=pfList.end();citp++)
   {
      const PassFailData& pfd = *citp;
      testComputeXvt(pfd,oas,testFramework);
   }
   testComputeXvt(oas, testFramework);


      //--- Test the getSVHealth( ) method --------------------------------
   currMethod = typeDesc + " OrbAlmStore.getSVHealth() ";
   TUCSM(currMethod);

   for (citp=pfList.begin();citp!=pfList.end();citp++)
   {
      const PassFailData& pfd = *citp;
      testGetSVHealth(pfd,oas,testFramework);
   }
   testGetSVHealth(oas, testFramework);
   ...
   for (int i=0;i<3;i++)
   {
      try
      {
         ...
         {
            case 0:
               ss << "getXvt_WithinValidity() succeeded (correctly) for time"
                  << " earlier than fit interval.";
               TUFAIL(ss.str());
               break;
            case 1:
               ss << "getXvt_WithinValidity() succeeded for time within fit"
                  << " interval.";
               TUPASS(ss.str());
               ...
               {
                  TUFAIL("getXvt() and getXvt_WithinValidity() did not return"
                         " matching results.");
               }
               break;
            case 2:
               ss << "getXvt_WithinValidity() succeeded (correctly) for time"
                  << " later than fit interval.";
               TUFAIL(ss.str());
               break;
         }
      }
      catch(InvalidRequest ir)
      {
         ...
         {
            case 0:
               ss << "getXvt_WithinValid() failed (correctly) for time earlier"
                  << " than fit interval.";
               TUPASS(ss.str());
               break;
            case 1:
               ss << "getXvt_WithinValid() failed for time within fit"
                  << " interval.";
               TUFAIL(ss.str());
               break;
            case 2:
               ss << "getXvt_WithinValid() failed (correctly) for time later"
                  << " than fit interval.";
               TUPASS(ss.str());
               break;
         }
...
//-----------------------------------------------------------------------------
// Test OrbAlmStore.deriveLastXmit( ) with a set of arguments that are
// expected to pass
void OrbAlmStore_T::
   ...
   const std::string LNavEx[] =
      {
         ...
         "365,12/31/2015,00:02:18,1877,345738,1,63,425, 0x22C3550A, 0x1C230C58, 0x1FEE6CC4, 0x2AEAEEC0, 0x26A66A75, 0x2A666666, 0x26EEEE53, 0x2AEA4013, 0x0000003F, 0x0000006C",
            //
            // Next two records are designed to verify that the
            // EARLIEST transmit time is being retained.  The first is
            // hand-edited to show a later transmit time, but the same
            // data as the original.  The original record (transmitted
            // at 00:04:54) is then provided as the second record.  It
            // should be used in place of the first record.
         ...
         "365,12/31/2015,00:04:54,1877,345894,1,63,505, 0x22C3550A, 0x1C264D08, 0x114964A0, 0x1EC0910D, 0x3F52803B, 0x28434E0D, 0x0F4A2471, 0x04F2B1F2, 0x274ABF25, 0x3AC009BC",
            //
            // Next record is designed to verify that a later
            // re-transmission found in the loaded data will NOT
            // overwrite an earlier transmission of the same record
            // already stored in the store.  The record has been
            // hand-modified to have a LATER transmit time than the
            // PRN 5 almanac already stored.
...
void OrbAlmStore_T ::
testComputeXvt(const PassFailData& pfd,
               OrbAlmStore& oas,
               TestUtil& testFramework)
{
   gpstk::Xvt rv;
...
void OrbAlmStore_T ::
testGetSVHealth(const PassFailData& pfd,
                OrbAlmStore& oas,
                TestUtil& testFramework)
{
...
testUnhealthyLNav()
{
   ...
   gpstk::PackedNavBits msg = getPnbLNav(
      ...
      " 0x238FF0AB, 0x0D800704");
   unhealthyDataList.push_back(msg);
   oas.setDebugLevel(debugLevel);
   unsigned short retval = oas.addMessage(msg);
   TUCSM("addMessage");
   TUASSERTE(unsigned short, OrbAlmStore::ADD_XMIT + OrbAlmStore::ADD_SUBJ,
             retval);
   TUCSM("size()");
   TUASSERTE(unsigned, 2, oas.size());
   TUCSM("size(1)");
   TUASSERTE(unsigned, 1, oas.size(1));
   TUCSM("size(2)");
...
int main()
{
   ...
   errorTotal += testClass.testUnhealthyLNav();
