...
namespace gpstk
{
      ...
      void AntennaStore::addAntenna(std::string name, AntexData& antdata) throw(Exception)
      {
         if(!antdata.isValid()) {
      ...
      bool AntennaStore::getAntenna(std::string name, AntexData& antdata) throw()
      {
         std::map<std::string, AntexData>::iterator it;
         it = antennaMap.find(name);
      ...
      {
         std::map<std::string, AntexData>::const_iterator it;
      ...
      void AntennaStore::getNames(std::vector<std::string>& names) throw()
      {
         names.clear();
         std::map<std::string, AntexData>::iterator it;
         for(it = antennaMap.begin(); it != antennaMap.end(); it++)
      ...
      void AntennaStore::getReceiverNames(std::vector<std::string>& names) throw()
      {
         names.clear();
         std::map<std::string, AntexData>::iterator it;
         for(it = antennaMap.begin(); it != antennaMap.end(); it++) {
      ...
      void AntennaStore::includeReceivers(std::vector<std::string>& names) throw()
      {
      ...
      int AntennaStore::addANTEXfile(std::string filename,
                       CommonTime time)
         throw(Exception)
      {
         try {
      ...
      /// Satellites are identified by two things:
      /// system character: G or blank GPS, R GLONASS, E GALILEO,
      /// C BeiDou
      /// and integer PRN or SVN number.
      /// NB. PRNs apply to GLONASS as well as GPS
      /// NB. Typically there is more than one antenna per satellite in ANTEX files;
         ...
         throw(Exception)
      {
         ...
         try
         {
            if (getSatelliteAntenna(sys, n, name, antenna))
            {
               ...
               // N.B.:  This following section is full of "magic numbers" and details that 
               // should get incorporated in a system-specific object for better documentation
              ...
              // Use low accuracy Sun vector 
              // from SunEarthSatGeometry
              double AR;     // angular radius of sun
              double shadowFactorDummy = 1;
              SVAtt = SatelliteAttitude(ct, satVector, shadowFactorDummy);
      ...
      /// dump the store
      void AntennaStore::dump(std::ostream& s, short detail)
      {
