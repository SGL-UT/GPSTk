 ...
 * gpstk::NovatelData - container for Novatel data, with methods to
 * convert to Rinex
 */
...
namespace gpstk
{
      ...
      /** Read Novatel binary records and parse into Rinex. Currently
       * supports the basic data records of OEM2 and OEM4 files only;
       * others are yet to be implemented.
       * Ref. (OEM2) 'GPSCard Command Descriptions' Manual (Rev 3)
       * Ref. 'OEM4 Family of Receivers,' Users Manual Volume 2
       *   (OM-20000047 Rev 12) */
   ...
   public:
      ...
      virtual ~NovatelData(void) {}
         ...
         /** Return true if this is a valid Novatel record.  Test the
          * validity of the record with this before further
          * processing. */
      ...
      virtual bool isData(void) const { return true; }
         ...
         /** True if this record is an ephemeris record.  Test the
          * identity of the record with this before casting into
          * Rinex. */
      bool isNav(void) const;

         /** True if this record is observation data.  Test the
          * identity of the record with this before casting into
          * Rinex. */
      ...
      operator RinexNavData() throw(gpstk::Exception);
          ...
          * @throw if the record is invalid or not an observation
          *   (isObs()==false) */
   ...
   private:
      ...
      unsigned char buffer[65564];   ///< buffer for raw data
         ...
         /** Reference GPS week, for OEM2, where the nav records
          * require a GPS week, but only the obs records have one, and
          * then it is 10-bit.  This epoch will be used to remove the
          * ambiguity in the 10-bit week number of the obs records,
