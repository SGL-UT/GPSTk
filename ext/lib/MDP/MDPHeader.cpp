...
namespace sgltk
{
   ...
   string MDPHeader::encode() const
      throw()
   {
     short week=static_cast<gpstk::GPSWeekSecond>(time).week;
      unsigned long sow100=static_cast<unsigned long>(
         0.5 + static_cast<gpstk::GPSWeekSecond>(time).sow * 100);
      if (sow100==60480000)
      {
         sow100=0;
         week++;
      ...
      throw()
   {
      ...
      time=gpstk::GPSWeekSecond( week, double(sow100)/100, gpstk::TimeSystem::GPS );

      clearstate( gpstk::fmtbit );

      // only clear the this bit if this object isn't a leaf data member
      if (id==0)
         clearstate( gpstk::crcbit );
   }
      ...
      throw( gpstk::FFStreamError )
   {
      ...
      {
         gpstk::FFStreamError e( "Message Length should be >= " +
                                 asString(length) +
                                 ".  Was: " +
                                 asString(str.length()) );
         GPSTK_THROW(e);
   ...
   //---------------------------------------------------------------------------
   void MDPHeader::reallyPutRecord( gpstk::FFStream& ffs ) const
             ...
             gpstk::FFStreamError )
   {
      ...
      {
         cout << endl;
         gpstk::StringUtils::hexDumpData(cout, str);
      }
   ...
   void MDPHeader::readHeader(MDPStream& ffs)
      throw(gpstk::FFStreamError, gpstk::EndOfFile)
   {
      ...
      if ((hexDump || debugLevel) && i>0)
      {
         ...
         if (debugLevel>1)
            gpstk::StringUtils::hexDumpData(cout, s);
      }
      ...
      if (debugLevel>2)
      {
         ...
         gpstk::StringUtils::hexDumpData(cout, ffs.rawHeader);
      }
   }
   ...
   //---------------------------------------------------------------------------
   void MDPHeader::reallyGetRecord( gpstk::FFStream& ffs )
      throw( std::exception, gpstk::StringUtils::StringException,
             gpstk::FFStreamError, gpstk::EndOfFile )
   {
      ...
      clear( gpstk::fmtbit | gpstk::lenbit | gpstk::crcbit );
      ...
      if (reqId == 0)
      {
         setstate( gpstk::crcbit|gpstk::lenbit|gpstk::fmtbit );
         return;
      }
      ...
      if (hexDump || (debugLevel>1 && rdstate()))
      {
         ...
         gpstk::StringUtils::hexDumpData(cout, stream.rawHeader+body);
      }
