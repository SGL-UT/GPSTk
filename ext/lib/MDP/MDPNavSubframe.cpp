...
namespace sgltk
{
      ...
      throw()
   {
      ...
      carrier = static_cast<gpstk::CarrierCode>(decodeVar<int8_t>(str));
      range   = static_cast<gpstk::RangeCode>  (decodeVar<int8_t>(str));
      nav     = static_cast<gpstk::NavCode>    (decodeVar<int8_t>(str));
   ...
   //--------------------------------------------------------------------------
   void MDPNavSubframe::uprightSubframe() throw()
   {
   ...
   bool MDPNavSubframe::checkParity() const throw()
   {
      ...
      for (int i=1; i<=10; i++)
      {
         ...
         unsigned computedParity = gpstk::EngNav::computeParity(curr, prev, cooked);
         if (debugLevel>3)
