...
#pragma clang diagnostic pop
      ...
      throw()
   {
      ...
      str += encodeVar( (uint32_t) 100*(static_cast<gpstk::GPSWeekSecond>(selfTestTime).sow));
      str += encodeVar( (uint16_t)      static_cast<gpstk::GPSWeekSecond>(selfTestTime).week);
      str += encodeVar( (uint16_t)      static_cast<gpstk::GPSWeekSecond>(firstPVTTime).week);
      str += encodeVar( (uint32_t) 100*(static_cast<gpstk::GPSWeekSecond>(firstPVTTime).sow));
      ...
      throw()
   {
      ...
      selfTestTime  = gpstk::GPSWeekSecond( week, double(sow100)*0.01 );
      week          = decodeVar<uint16_t>(str);
      sow100        = decodeVar<uint32_t>(str);
      firstPVTTime  = gpstk::GPSWeekSecond( week, double(sow100)*0.01 );
