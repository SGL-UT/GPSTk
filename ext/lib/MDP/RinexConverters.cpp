...
namespace sgltk
{
   ...
   gpstk::RinexObsData::RinexObsTypeMap makeRinexObsTypeMap(const MDPObsEpoch& moe) throw()
   {
      ...
      j = ol.find(MDPObsEpoch::ObsKey( gpstk::ccL1, gpstk::rcCA ));
      if (j!=ol.end())
      {
      ...
      j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL1, gpstk::rcYcode));
      if (j == ol.end())
         j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL1, gpstk::rcPcode));
      if (j == ol.end())
         j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL1, gpstk::rcCodeless));
      if (j != ol.end())
      {
         short lli = j->second.lockCount ? 0 : 1;
         short ssi = snr2ssi(j->second.snr);
      ...
      j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL2, gpstk::rcYcode));
      if (j == ol.end())
         j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL2, gpstk::rcPcode));
      if (j == ol.end())
         j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL2, gpstk::rcCodeless));
      if (j != ol.end())
      {
         short lli = j->second.lockCount ? 0 : 1;
         short ssi = snr2ssi(j->second.snr);
      ...
      // Now get the C2
      j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL2, gpstk::rcCM));
      if (j == ol.end())
         j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL2, gpstk::rcCL));
      if (j == ol.end())
         j = ol.find(MDPObsEpoch::ObsKey(gpstk::ccL2, gpstk::rcCMCL));
      if (j != ol.end())
      {
         short lli = j->second.lockCount ? 0 : 1;
         short ssi = snr2ssi(j->second.snr);
   ...
   gpstk::RinexObsData makeRinexObsData(const MDPEpoch& mdp)
   {
...
int week=(static_cast<gpstk::GPSWeekSecond>(sf4p18->second.time).week);
   ...
   bool makeEngEphemeris(gpstk::EngEphemeris& eph, const EphemerisPages& pages)
   {
      ...
      int week = 0;
      try {
         week = (static_cast<gpstk::GPSWeekSecond>(sf[1]->second.time).week);
      }
      catch (...)
      {}
      long sfa[10];
