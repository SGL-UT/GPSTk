...
namespace gpstk
{
   ...
   public:
      LNavFilterData();
         /** Must be set this before use in a filter.  The data is
          * expected to be an array of 10 uint32_t values where each
          * array index corresponds to a subframe word (sf[0] is word
          * 1, and so on).  The words are right-aligned meaning that
          * the LSB of word 1 is in the LSB of sf[0].
          *
