...
MDPScreenProcessor::MDPScreenProcessor(sgltk::MDPStream& in, std::ofstream& out):
   MDPProcessor(in, out),
...
void MDPScreenProcessor::process(const sgltk::MDPNavSubframe& sf)
{
...
void MDPScreenProcessor::process(const sgltk::MDPSelftestStatus& sts)
{
   currentSts = sts;
   drawSTS();
   redraw();
