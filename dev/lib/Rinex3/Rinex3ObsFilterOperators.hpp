 ...
 * Operators for FileFilter using Rinex3 observation data
 */
...
namespace gpstk
{
   ...
   {
   public:
         /// The set is a set of ObsID that the two files have in 
      ...
      (const std::set<gpstk::Rinex3ObsHeader::ObsID>& rohset)
            : obsSet(rohset)
         {}
      ...
      bool operator()(const gpstk::Rinex3ObsData& l,
                      const gpstk::Rinex3ObsData& r) const
         {
            ...
            if (l.obs.size() > r.obs.size())
               ...
               // shared fields
            gpstk::Rinex3ObsData::Rinex3SatMap::const_iterator lItr = 
               l.obs.begin(), rItr;
            ...
            while (lItr != l.obs.end())
            {
               ...
               gpstk::Rinex3ObsData::ObsIDMap 
                  lObs = (*lItr).second, 
                  rObs = (*rItr).second;

               std::set<gpstk::Rinex3ObsHeader::ObsID>::const_iterator obsItr = 
                  obsSet.begin();
         
               while (obsItr != obsSet.end())
               {
                  gpstk::Rinex3ObsData::Rinex3Datum lData, rData;
                  lData = lObs[*obsItr];
                  rData = rObs[*obsItr];
   ...
   private:
      std::set<gpstk::Rinex3ObsHeader::ObsID> obsSet;
   };
   ...
   public:
      ...
      /// and adding the appropriate ObsIDs.  This assumes that
      ...
      /// those files and obsSet will be the set of ObsIDs that
      ...
      bool operator()(const gpstk::Rinex3ObsHeader& l)
         {
            ...
            {
               std::set<gpstk::Rinex3ObsHeader::ObsID> thisObsSet, 
                  tempObsSet;
