...
namespace Rinex3
{
   ...
   bool CivilTime::setTimeFrame( const TimeFrame& tf )
   {
      timeFrame = tf;
   }
   ...
   bool CivilTime::operator<( const CivilTime& right ) const
      throw()
   {
      if( timeFrame != right.timeFrame )
      {
         return false; // cannot compare if not in same frame
      }
      if( year < right.year )
      {
         return true;
