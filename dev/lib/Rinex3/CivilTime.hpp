...
namespace Rinex3
{
       ...
       * year, month, day (of month), hour (of day), minute (of hour),
       * second (of minute), and time frame used (see TimeTag enum).
       */
   ...
   public:
      ...
      static const CivilTime END_OF_TIME;
         ...
         /**
          * Default Constructor.
          * All elements default to zero, time frame to Unknown.
          */
      CivilTime( int yr = 0,
                 int mo = 0,
                 int dy = 0,
         ...
         throw()
            : year(yr), month(mo), day(dy), hour(hr), minute(mn), second(s), timeFrame(tf)
      {}
      ...
      CivilTime( const CivilTime& right )
         throw()
            : year( right.year ), month( right.month ), day( right.day ),
              ...
              second( right.second ), timeFrame( right.timeFrame )
      {}
      ...
      virtual bool setTimeFrame( const TimeFrame& tf )
         throw();
