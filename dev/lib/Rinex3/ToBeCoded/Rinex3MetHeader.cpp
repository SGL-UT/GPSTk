...
namespace gpstk
{
   const int Rinex3MetHeader::maxObsPerLine = 9;

   const string Rinex3MetHeader::versionString = "RINEX3 VERSION / TYPE";
   const string Rinex3MetHeader::runByString = "PGM / RUN BY / DATE";
   const string Rinex3MetHeader::commentString = "COMMENT";
   const string Rinex3MetHeader::markerNameString = "MARKER NAME";
   const string Rinex3MetHeader::markerNumberString = "MARKER NUMBER";
   const string Rinex3MetHeader::obsTypeString = "# / TYPES OF OBSERV";
   const string Rinex3MetHeader::sensorTypeString = "SENSOR MOD/TYPE/ACC";
   const string Rinex3MetHeader::sensorPosString = "SENSOR POS XYZ/H";
   const string Rinex3MetHeader::endOfHeader = "END OF HEADER";


   
   std::string Rinex3MetHeader::bitString(unsigned long vb, char quote,
                                         std::string sep)
   {
      unsigned long b = 1;
      std::string rv;
   ...
   void Rinex3MetHeader::reallyPutRecord(FFStream& ffs) const
            ...
            gpstk::StringUtils::StringException)
   {
      ...
      if (valid & obsTypeValid)
      {
         ...
         vector<Rinex3MetType>::const_iterator itr = obsTypeList.begin();
         size_t numWritten = 0;
   ...
   void Rinex3MetHeader::reallyGetRecord(FFStream& ffs) 
      throw(std::exception, FFStreamError, 
            gpstk::StringUtils::StringException)
   {
      ...
      while (! (valid & endValid))
      {
         ...
         if (thisLabel == versionString)
         {
            ...
            if ( (fileType[0] != 'M') &&
                 (fileType[0] != 'm'))
            {
               FFStreamError e("This isn't a Rinex3 Met file");
               GPSTK_THROW(e);
            }
            valid |= versionValid;
         }
   ...
   void Rinex3MetHeader::dump(ostream& s) const
   {
   ...
   Rinex3MetHeader::Rinex3MetType 
   Rinex3MetHeader::convertObsType(const string& oneObs)
      throw(FFStreamError)
   {
   ...
   string Rinex3MetHeader::convertObsType(const Rinex3MetHeader::Rinex3MetType& oneObs)
      throw(FFStreamError)
   {
      if      (oneObs == PR) return "PR";
      else if (oneObs == TD) return "TD";
