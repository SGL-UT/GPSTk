 ...
 * Encapsulates RINEX3 Navigation data
 */
...
namespace gpstk
{
   ...
   Rinex3NavData::operator EngEphemeris() const throw()
   {
      EngEphemeris ee;
         ...
         // Also, in Rinex3, the accuracy is in meters, and setSF1 expects
      ...
      throw(StringException)
   {
         ...
         // In Rinex3 *files*, weeknum is the week of TOE
      double wk=double(weeknum);
      if(HOWtime - Toe > CommonTime::HALFWEEK)
         wk++;
      else if(HOWtime - Toe < -(CommonTime::HALFWEEK))
         wk--;
      ...
      throw(StringException, FFStreamError)
   {
      try
      {
         ...
         time = CommonTime(yr,mo,day,hr,min,sec);
         if(ds != 0) time += ds;
      ...
      throw(StringException, FFStreamError)
   {
      try
      {
         ...
         // Some Rinex3 files have HOW < 0
         while(HOWtime < 0) {
	   HOWtime += (long) CommonTime::FULLWEEK;
            weeknum--;
         }
