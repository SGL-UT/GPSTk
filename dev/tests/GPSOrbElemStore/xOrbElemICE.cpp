...
*  using OrbElemCNAV2 in order to calculate position, velocity, 
...
int main( int argc, char * argv[] )
{
   ...
   CivilTime g( 2011, 6, 2, 12, 0, 0.0, TimeSystem::GPS );
   ...
   CommonTime testT = GPSWeekSecond(1638, 378000+7200);
   testT.setTimeSystem(TimeSystem::GPS);
   short PRNIDArg = 3;
   int TOI = 50;   // 9 bit word from subframe 1
   OrbElemCNAV2 cec;
   cec.loadData( obsID, PRNIDArg, TOI, pnb);
   double elev = 30; // 30 degrees - sin(30) = 0.5
   for(int i = -17; i <= 16; i++)
   {
      cec.URAned0 = i;
      cec.URAed = i;
      ...
      try
      {
         double AdjNomURAed = cec.getAdjNomURAed(elev);
         double CompositeIAURA = cec.getCompositeIAURA(testT, elev);
         cout << "For i = " << dec << i << ": AdjNomURAed = " << AdjNomURAed
              << ", CompositeIURA = " << CompositeIAURA << " t - ctTop = " << testT-cec.ctTop
              << endl;
      }
      catch(InvalidRequest exe)
      {
         cout << "i = " << dec << i << ". " << exe.getText() << endl;
      }
   }  
   return(0);
}
