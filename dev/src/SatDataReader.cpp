...
namespace gpstk
{
    ...
    void SatDataReader::loadData(void) throw(FFStreamError, gpstk::StringUtils::StringException)
    {
        while(1) // Do this until end-of-file reached or something else happens
        {
        try {
            std::string line;
            ...
            if( StringUtils::firstWord(line)[0] == '#' ) formattedGetLine(line, true);
            std::string::size_type idx = line.find('#');
            if( !(idx == std::string::npos) ) line = line.substr(0, idx);

            // We erase the header (first line)
            if( StringUtils::firstWord(line) == "Launch" ) formattedGetLine(line, true);
            ...
            // marked as unknown
            SatID sat(sat.id = StringUtils::asInt(prn),SatID::systemUnknown);
            // Let's identify satellite system
            if(block[0] == 'I') sat.system = SatID::systemGPS;
            else 
                if (block.substr(0, 3) == "GLO") sat.system = SatID::systemGlonass;
     ...
     */
    string SatDataReader::getBlock(const SatID& sat, const DayTime& epoch)
    {
        ...
        if( (*iter).second.launchDate > epoch ) return "";

        // Increment iterator "iter" if we are not yet at proper epoch range
        while( (*iter).second.deactivationDate < epoch ) ++iter;
     ...
     */
    int SatDataReader::getGPSNumber(const SatID& sat, const DayTime& epoch)
    {
        ...
        if( (*iter).second.launchDate > epoch ) return -1;
     ...
     */
    DayTime SatDataReader::getLaunchDate(const SatID& sat, const DayTime& epoch)
    {
        ...
        if(range.first == range.second) return DayTime::END_OF_TIME;

        // Declare an iterator to travel in this range
        satDataIt iter(range.first);

        // If this epoch is before launch date, return DayTime::END_OF_TIME
        if( (*iter).second.launchDate > epoch ) return DayTime::END_OF_TIME;
     ...
     */
    DayTime SatDataReader::getDeactivationDate(const SatID& sat, const DayTime& epoch)
    {
        ...
        if(range.first == range.second) return DayTime::BEGINNING_OF_TIME;

        // Declare an iterator to travel in this range
        satDataIt iter(range.first);

        // If this epoch is before launch date, return BEGINNING_OF_TIME
        if( (*iter).second.launchDate > epoch ) return DayTime::BEGINNING_OF_TIME;
