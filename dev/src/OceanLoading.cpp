...
namespace gpstk
{
     ...
     */
    Triple OceanLoading::getOceanLoading(const string& name, const DayTime& t) throw(InvalidRequest)
    {
    ...
    Vector<double> OceanLoading::getArg(const DayTime& time)
    {
        ...
        for(int k=0; k<11; k++)
        {
            ...
            if (arguments(k) < 0.0) arguments(k) = arguments(k) + TWO_PI;
        }
