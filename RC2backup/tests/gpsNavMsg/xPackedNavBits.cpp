...
/*   pnb.addUnsignedLong(u_i1, u_n1, u_s1);
   ...
   /* Unpack */
 /*  cout << endl;
   cout << "Unpacked Unsigned Integers:" << endl;
   int startbit = 0;
   cout << "Number 32767:  " << pnb.asUnsignedLong(startbit, u_n1, u_s1) << endl;
   startbit += u_n1;
   cout << "Number 1:      " << pnb.asUnsignedLong(startbit, u_n2, u_s2) << endl;
   startbit += u_n2;
   cout << "Number 255:    " << pnb.asUnsignedLong(startbit, u_n3, u_s3) << endl;
   startbit += u_n3;
   cout << "Number 604500: " << pnb.asUnsignedLong(startbit, u_n4, u_s4) << endl;
   startbit += u_n4;
   cout << endl;

   cout << "Unpacked Signed Integers: " << endl;
   cout << "Number 15:      " << pnb.asLong(startbit, s_n1, s_s1) << endl;
   startbit += s_n1;
   cout << "Number -16:     " << pnb.asLong(startbit, s_n2, s_s2) << endl;
   startbit += s_n2;
   cout << "Number -1:      " << pnb.asLong(startbit, s_n3, s_s3) << endl;
   startbit += s_n3;
   cout << "Number 0:       " << pnb.asLong(startbit, s_n4, s_s4) << endl;
   startbit += s_n4;
   cout << "Number 4194304: " << pnb.asLong(startbit, s_n5, s_s5) << endl;
   startbit += s_n5;
   cout << endl;

   cout << "Unpacked Signed Doubles: " << endl;
   cout << "Number 0.490005493: " << pnb.asSignedDouble(startbit, d_n1, d_s1) << endl;
   startbit += d_n1;
   cout << "Number -0.5:        " << pnb.asSignedDouble(startbit, d_n2, d_s2) << endl;
   startbit += d_n2;
   cout << "Number 0:           " << pnb.asSignedDouble(startbit, d_n3, d_s3) << endl;
   startbit += d_n3;
   cout << endl;

   cout << "Unpacked Unsigned Doubles: " << endl;
   cout << "Number 32000.0: " << pnb.asUnsignedDouble(startbit, d_n4, d_s4) << endl;
   startbit += d_n4;
   cout << endl;

   cout << "Unpacked Double Semi-Circles: " << endl;
   cout << "Number PI:  " << pnb.asDoubleSemiCircles(startbit, sd_n1, sd_s1) << endl;
   startbit += sd_n1;
   cout << "Number -PI: " << pnb.asDoubleSemiCircles(startbit, sd_n2, sd_s2) << endl; */
   ...
   pnb.addDoubleSemiCircles(ridot, n_ridot, s_ridot); 
      ...
      // Test Data copied from RINEX file for PRN3, week 1638, day 153 2011
   int startbit = 0;
   cout << "Tgd:      " << pnb.asSignedDouble(startbit, n_rTgd, s_rTgd) << endl;
   startbit += n_rTgd;
   cout << "IODC:     " << pnb.asUnsignedLong(startbit, n_riodc, s_riodc) << endl;
   startbit += n_riodc;
   cout << "Toc:      " << pnb.asUnsignedDouble(startbit, n_rToc, s_rToc) << endl;
   startbit += n_rToc;
   cout << "af2:      " << pnb.asSignedDouble(startbit, n_raf2, s_raf2) << endl;
   startbit += n_raf2;
   cout << "af1:      " << pnb.asSignedDouble(startbit, n_raf1, s_raf1) << endl;
   startbit += n_raf1;
   cout << "af0:      " << pnb.asSignedDouble(startbit, n_raf0, s_raf0) << endl;
   startbit += n_raf0;
   cout << "IODE:     " << pnb.asUnsignedLong(startbit, n_riode, s_riode) << endl;
   startbit += n_riode;
   cout << "Crs:      " << pnb.asSignedDouble(startbit, n_rCrs, s_rCrs) << endl;
   startbit += n_rCrs;
   cout << "dn:       " << pnb.asDoubleSemiCircles(startbit, n_rdn, s_rdn) << endl;
   startbit += n_rdn;
   cout << "M0:       " << pnb.asDoubleSemiCircles(startbit, n_rM0, s_rM0) << endl;
   startbit += n_rM0;
   cout << "Cuc:      " << pnb.asSignedDouble(startbit, n_rCuc, s_rCuc) << endl;
   startbit += n_rCuc;
   cout << "ecc:      " << pnb.asUnsignedDouble(startbit, n_recc, s_recc) << endl;
   startbit += n_recc;
   cout << "Cus:      " << pnb.asSignedDouble(startbit, n_rCus, s_rCus) << endl;
   startbit += n_rCus;
   cout << "Ahalf:    " << pnb.asUnsignedDouble(startbit, n_rAhalf, s_rAhalf) << endl;
   startbit += n_rAhalf;
   cout << "Toe:      " << pnb.asUnsignedDouble(startbit, n_rToe, s_rToe) << endl;
   startbit += n_rToe;
   cout << "fitInt:   " << pnb.asUnsignedLong(startbit, n_rfitInt, s_rfitInt) << endl;
   startbit += n_rfitInt;
   cout << "AODO:     " << pnb.asUnsignedLong(startbit, n_raodo, s_raodo) << endl;
   startbit += n_raodo;
   cout << "Cic:      " << pnb.asSignedDouble(startbit, n_rCic, s_rCic) << endl;
   startbit += n_rCic;
   cout << "OMEGA0:   " << pnb.asDoubleSemiCircles(startbit, n_rOMEGA0, s_rOMEGA0) << endl;
   startbit += n_rOMEGA0;
   cout << "Cis:      " << pnb.asSignedDouble(startbit, n_rCis, s_rCis) << endl;
   startbit += n_rCis;
   cout << "i0:       " << pnb.asDoubleSemiCircles(startbit, n_ri0, s_ri0) << endl;
   startbit += n_ri0;
   cout << "Crc:      " << pnb.asSignedDouble(startbit, n_rCrc, s_rCrc) << endl;
   startbit += n_rCrc;
   cout << "w:        " << pnb.asDoubleSemiCircles(startbit, n_rw, s_rw) << endl;
   startbit += n_rw;
   cout << "OMEGAdot: " << pnb.asDoubleSemiCircles(startbit, n_rOMEGAdot, s_rOMEGAdot) << endl;
   startbit += n_rOMEGAdot;
   cout << "IODE:     " << pnb.asUnsignedLong(startbit, n_riode, s_riode) << endl;
   startbit += n_riode;
   cout << "idot:     " << pnb.asDoubleSemiCircles(startbit, n_ridot, s_ridot) << endl;
