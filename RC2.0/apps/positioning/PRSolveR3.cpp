   ...
   if (C.InputMetName.size() > 0)
   {
      ...
      if (C.Debug)
      {
         ...
         for ( ; it != C.MetStore.end(); it++)
         {
            ...
            C.oflog << civtime.printf("%02m/%02d/%04Y %02H:%02M:%02S %P")
                    << fixed << setprecision(1);
            cout << civtime.printf("%02m/%02d/%04Y %02H:%02M:%02S %P");
...
int ReadFile(int nfile) throw(Exception)
{
   try
   {
      ...
      Rinex3ObsStream ifstr, ofstr; // input and output RINEX 3 files
...
//      cout << "Reading Files..." << endl;
            ...
            || hasC2P == 0 || hasL1 == 0 || hasL2 == 0 )
      {
         ...
         if (hasC1C == 0) C.oflog << " C1" << " (forceCA is " << (C.ForceCA?"T":"F") << ")";
      ...
      if (C.ForceCA)
      {
         ...
         else
         {
            ...
            cout << "ERROR. Abort. --forceCA was found but C1 data is not found." << endl;
            return -1;
         }
      }
      else if (hasC1P == 0)
      {
         ...
         else if (C.UseCA && hasC1C == 0)
         {
            ...
            cout << "ERROR. Abort. Neither P1 nor C1 data found (--useCA is set)." << endl;
            return -1;
         }
         else if (C.Freq != 2 && !C.UseCA && hasC1C > 0)
         {
            C.oflog << "ERROR. Abort. P1 data not found (C1 data found: add --useCA)" << endl;
            cout << "ERROR. Abort. P1 data not found (C1 data found: add --useCA)" << endl;
            return -1;
         }
         else if (C.Freq != 2)
         {
            C.oflog << "ERROR. Abort. Neither P1 nor C1 data found." << endl;
            cout << "ERROR. Abort. Neither P1 nor C1 data found." << endl;
      ...
      while(1)
      {
         ...
         if ( prsol.Solution.size() == 5 )
            SR[3].Add(prsol.Solution(4)/C_MPS*1e9); // GPS-GLO time offset (ns), mixed case only
         SSR[0].Add(prsol.Solution(0));
         SSR[1].Add(prsol.Solution(1));
         SSR[2].Add(prsol.Solution(2));
         if ( prsol.Solution.size() == 5 )
         {
            SSR[3].Add(prsol.Solution(4)/C_MPS*1e9); // GPS-GLO time offset (ns), mixed case only
         }
         ...
         // output to RINEX
         if (first)           // edit output RINEX header
         {
            rheadout = rhead;
   ...
   throw(Exception)
{
   ...
   try
   {
      ...
      if (C.Debug)
      {
         C.oflog << "Beginning autonomous PR solution. " << endl << endl;
      }
      ...
      if (C.APSout)
      {
         ...
         if (iret == -4)
         {
            if (C.Debug)
            {
               ...
               C.oflog << "Line: " << __LINE__ << endl << endl;
            }
...
//                 << " " << (static_cast<CivilTime>(CurrEpoch)).printf(C.timeFormat)
      ...
      if(prsol.Solution.size() == 5)
      {
         ...
         cout << sod << " " << ( prsol.Solution(4) / C_MPS * 1.0e9 ) << endl;
      }
...
int GetCommandLine(int argc, char **argv) throw(Exception)
{
   try {
      ...
      {
         cout << "\nErrors found in command line input:" << endl;
         Par.dumpErrors(cout);
         cout << "...end of Errors" << endl << endl;
         help = true;
      }
      ...
      if (dashn.getCount())
      {
         ...
         if (help) cout << "Input RINEX nav files are:" << endl;
         for (i=0; i<values.size(); i++)
         {
      ...
      if (dashm.getCount())
      {
         ...
         if (help) cout << "Input RINEX met files are:" << endl;
         for (i=0; i<values.size(); i++)
         {
      ...
      {
         C.UseCA = true;
         if (help) cout << "'Use C/A' flag is set" << endl;
      }
      if (dashfCA.getCount())
      {
         C.ForceCA = true;
         /**if (help)**/ cout << "'Force C/A' flag is set" << endl;
      }
      ...
      {
         C.algebra = true;
         if (help) cout << "'Algebraic' option is on" << endl;
      }
      if (dashrcrt.getCount())
      {
         C.residCrit = false;
         if (help) cout << "'ResidualCriterion' option is false" << endl;
      }
      if (dashrone.getCount())
      {
         C.returnatonce = true;
         if (help) cout << "'Return at once' option is true" << endl;
      }
...
void DumpConfiguration(ostream& os) throw(Exception)
{
   try
   {
      ...
      os << " # Input: " << endl;
      if (C.useGPS)
        os << " System Used: GPS" << endl;
      else if (C.useGlonass)
        os << " System Used: GLONASS" << endl;
      else
        os << " System Used: All" << endl;
      ...
      else os << " No input meteorological data" << endl;
      ...
      if (C.Tend < CommonTime::END_OF_TIME)
         os << " End time is "
            ...
            << " = " << C.Tend << endl;
      if (C.UseCA) os << " 'Use C/A' flag is set" << endl;
      if (C.ForceCA) os << " 'Force C/A' flag is set" << endl;
      ...
      if (C.knownpos.getCoordinateSystem() != Position::Unknown)
         os << " Output residuals: known position is   " << endl
            << C.knownpos.printf("ECEF(m) %.4x %.4y %.4z     = %A deg N %L deg E %h m") << endl;
      if (!C.ordFile.empty())
