...
#ifndef LIB_ORD_ORD_HPP_
#define LIB_ORD_ORD_HPP_
...
namespace gpstk
{
	namespace ord
	{
		// Given a set of frequency and pseudorange pairs, attempts to compensate for ionospheric effects.
		// Comment: Vectors are used because they carry a size parameter, another collection type may be more appropriate.
		// Parameters:
		//		frequencies (vector double): Signal frequencies.
	    //      pseudoranges (vector double): Pseudorange values, corresponding to frequency array.
		...
		double IonosphereModelCorrection(const gpstk::IonoModelStore& ionoModel, const gpstk::Position& rxLoc, const gpstk::Xvt& svXvt);

		// Given a satellite id, a time, and an ephemeris store, retrieves the satellite location/velocity in xvt instance.
		// Note: This is a relatively thin wrapper for XvtStore.getXvt() to bring the method into the same namespace as the
		//       other range calculations.
		// Parameters:
		...
		gpstk::Xvt getSvXvt(const gpstk::SatID& sat_id, const gpstk::CommonTime& time, const gpstk::XvtStore<gpstk::SatID>& ephemeris);
		...
		double RawRange1(const gpstk::Position& rx_loc, const gpstk::SatID& sat_id, const gpstk::CommonTime& time,
		...
		//      (primary) Range in meters
		//		sv_xvt (Xvt): Satellite location/velocity
		double RawRange2(double pseudorange, const gpstk::Position& rx_loc, const gpstk::SatID& sat_id, const gpstk::CommonTime& time,
				const gpstk::XvtStore<gpstk::SatID>& ephemeris, gpstk::Xvt& sv_xvt);
		...
		//      (primary) Range in meters
		//		sv_xvt (Xvt): Satellite location/velocity
		double RawRange3(double pseudorange, const gpstk::Position& rx_loc, const gpstk::SatID& sat_id, const gpstk::CommonTime& time,
				const gpstk::XvtStore<gpstk::SatID>& ephemeris, gpstk::Xvt& sv_xvt);
		...
		//      (primary) Range in meters
		//		sv_xvt (Xvt): Satellite location/velocity
		double RawRange4(const gpstk::Position& rx_loc, const gpstk::SatID& sat_id, const gpstk::CommonTime& time,
				const gpstk::XvtStore<gpstk::SatID>& ephemeris, gpstk::Xvt& sv_xvt);
		...
		//      sv_loc (Xvt): The location of the satellite at time of interest.
		// Returns: range correction (delta) in meters
		double TroposphereCorrection(const gpstk::TropModel& trop_model, const gpstk::Position& rx_loc, const gpstk::Xvt& sv_xvt);

		// Example method that applies _all_ corrections to generate the Observed Range Deviation.
		...
		//      ephemeris (XvtStore): The ephemeris to query against.
		// Returns:
		//      (double) ORD - Observed range deviation from pseudorange (or effective pseudorange if multiple are provided)
		double calculate_ord(const std::vector<double>& frequencies, const std::vector<double>& pseudoranges,
				const gpstk::Position& rx_loc, const gpstk::SatID& sat_id,
				const gpstk::CommonTime& transmit_time, const gpstk::CommonTime& receive_time,
				const gpstk::IonoModelStore& iono_model, const gpstk::TropModel& trop_model, const gpstk::XvtStore<gpstk::SatID>& ephemeris,
				int range_method)
		{
			...
			switch(range_method)
			{
				case 1:
					...
					break;
				case 2:
					range = RawRange2(ps_range, rx_loc, sat_id, receive_time, ephemeris, sv_xvt);
					break;
				case 3:
					range = RawRange3(ps_range, rx_loc, sat_id, transmit_time, ephemeris, sv_xvt);
					break;
				case 4:
