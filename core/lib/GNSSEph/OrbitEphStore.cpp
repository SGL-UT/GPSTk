 ...
 * Class for storing and/or computing position, velocity, and clock
 * data using tables of <SatID, <time, OrbitEph> >, initial and final
 * times and search methods.  Note this class may be used as is, using
 * OrbitEph rather than its descendants, without health or accuracy
 * information; however most likely the user will define a class that
 * inherits this and uses one the classes derived from OrbitEph, for
 * example GPSEphemerisStore and GPSEphemeris. */
...
namespace gpstk
{
   ...
   Xvt OrbitEphStore::getXvt(const SatID& sat, const CommonTime& t) const
   {
      try
      {
         ...
         if(!eph)
         {
            ...
            // not have health); derived classes should override
            // isHealthy()
      ...
      try
      {
         ...
         if(it != toet.end())
         {
            ...
            else
            {
               ...
               string str = "Unexpected matching beginValid time but not Toe,"
                  " for "
                  + asString(eph->satID)
            ...
            // N.B:: lower_bound will return element beyond key since
            // there is no match
   ...
   {
      for(SatTableMap::iterator i = satTables.begin(); i != satTables.end();
          i++)
      {
         ...
         if(lower != eMap.begin())
         {
            for (TimeOrbitEphTable::iterator emi = eMap.begin(); emi != lower;
                 emi++)
            {
               delete emi->second;
            }
         ...
         if(upper != eMap.end())
         {
            for (TimeOrbitEphTable::iterator emi = upper; emi != eMap.end();
                 emi++)
            {
               delete emi->second;
            }
   ...
   unsigned OrbitEphStore::size(const SatID& sat) const
   {
      ...
      //-----------------------------------------------------------------------
      // The goal of this routine is to find the set of orbital
      // elements that would have been used by a receiver in
      // real-time. That is to say, the most recently broadcast
      // elements (assuming receiver has visibility to the given
      // satellite).
                                                   ...
                                                   const CommonTime& t) const
   {
      ...
      const TimeOrbitEphTable& table = getTimeOrbitEphMap(sat);
         ...
         // with a key "just beyond t" assuming the t is NOT a direct
         // match for any key.
      ...
      if(it == table.end())
      {
         ...
         it = table.lower_bound(t);
            ...
            // the table, lower_bound() will return
            // table.end(). However, this doesn't entirely settle the
            // matter. It is theoretically possible that the final
            // item in the table may have an effectivity that
