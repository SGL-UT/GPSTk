...
/** @file TabularSatStore.hpp Store a tabular list of data (for
 * example precise ephemeris data from an SP3 file) for several
 * satellites, and compute values at any timetag from this table. */
...
namespace gpstk
{
   ...
   protected:
         ...
         /** the data tables: std::map<SatID, std::map<CommonTime,
          * DataRecord> > */
      SatTable tables;
      ...
      bool checkInterval;
          ...
          * Lagrange interpolation, this should be (10-1)*900s+1=
          * 8101s. */
      double maxInterval;
   ...
   public:
      ...
      virtual DataRecord getValue(const SatID& sat, const CommonTime& ttag)
          ...
          * satellite.  Return two const iterators it1 and it2 (it1 <
          * it2) giving the range of 2*nhalf points, nhalf on each
          * side of the given time.
          * @note that a range is returned even if the input time
          * exactly matches one of the times in the table; in this
          * 'exact match' case the matching time will be at either
          * it1->first (if input parameter exactReturn is true) or
          * (it1+nhalf-1) or (it1+nhalf) (if exactReturn is false).
          * This routine is used to select data from the table for
          * interpolation; note that DataTable is a map<CommonTime,
          * DataRecord>.
          ...
          * @return bool: true if ttag matches a time in the table and
          ...
          * @throw the satellite is not found in the tables, or there
          *   is inadequate data
          * @throw GapInterval is set and there is a data gap larger
          *   than the max
         ...
         const throw(InvalidRequest)
      {
         try
         {
            ...
            if(--it1 == dtable.begin())
            {
               ...
               InvalidRequest
                  e("Inadequate data before(2) requested time for satellite "
                    + gpstk::StringUtils::asString(sat) + printTime(ttag,fmt));
               GPSTK_THROW(e);
            }
         ...
         const throw(InvalidRequest)
      {
         try {
            ...
            static const char *fmt=" at time %4Y/%02m/%02d %2H:%02M:%02S";
               ...
               // find the timetag in this table
               // @note. throw here if time systems do not match and are not "Any"
            it1 = dtable.find(ttag);
               // is it an exact match?
            bool exactMatch(it1 != dtable.end());
            ...
            if(detail > 0) {
               ...
               for(it=tables.begin(); it!=tables.end(); it++) {
                  ...
                  if(detail == 1) { os << std::endl; continue; }
                     ...
                     // @note cannot access DataRecord without operator<<(DataRecord)
                  os << "   Data:" << std::endl;
      ...
      CommonTime getFinalTime(const SatID& sat) const throw()
      {
          ...
          * @note the interval includes both it1 and it2. */
