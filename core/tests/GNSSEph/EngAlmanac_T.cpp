...
*/
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getEcc(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.ecc[i])*1E2 < eps))
				failCount++;
		}
		testMesg = "getEcc returned the wrong value";
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getIOffset(gpstk::SatID(x, gpstk::SatID::systemGPS)) - (aData.oi[i] - .3*M_PI)) < eps))
				failCount++;
		}
		testMesg = "getIOffset returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getOmegadot(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.rora[i])*1E8 < eps))
				failCount++;
		}
		testMesg = "getOmegadot returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(dataStore.get6bitHealth(gpstk::SatID(x, gpstk::SatID::systemGPS)) == 0))
				failCount++;
		}
		testMesg = "get6bitHealth returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);	
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(dataStore.getSVHealth(gpstk::SatID(x, gpstk::SatID::systemGPS)) == 0))
				failCount++;
		}
		testMesg = "getSVHealth returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);	
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(dataStore.getSVConfig(gpstk::SatID(x, gpstk::SatID::systemGPS)) == 9))
				failCount++;
		}
		testMesg = "getSVConfig returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);	
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getAhalf(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.sqrta[i])*1E-4 < eps))
				failCount++;
		}
		testMesg = "getAhalf returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);	
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getA(gpstk::SatID(x, gpstk::SatID::systemGPS)) - pow(aData.sqrta[i],2))*1E-8 < eps))
				failCount++;
		}
		testMesg = "getA returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getOmega0(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.raaw[i])*1E-1 < eps))
				failCount++;
		}
		testMesg = "getOmega0 returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getW(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.aop[i]) < eps))
				failCount++;
		}
		testMesg = "getW returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getM0(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.ma[i])*1E-1 < eps))
				failCount++;
		}
		testMesg = "getM0 returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getAf0(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.af0[i])*1E5 < eps))
				failCount++;
		}
		testMesg = "getAf0 returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
		...
		for (int i=0; i<31; i++)
		{
			...
			if (!(std::abs(dataStore.getAf1(gpstk::SatID(x, gpstk::SatID::systemGPS)) - aData.af1[i]) < eps))
				failCount++;
		}
		testMesg = "getAf1 returned the wrong value";
		testFramework.assert(failCount == 0, testMesg, __LINE__);
