...
//=======================================================================
   ...
   void subframe1Check(gpstk::EngEphemeris dataStore, gpstk::TestUtil& testFramework,
   ...
   void subframe2Check(gpstk::EngEphemeris dataStore, gpstk::TestUtil& testFramework,
                       bool skipASAlert = false)
   {
   ...
   void subframe3Check(gpstk::EngEphemeris dataStore, gpstk::TestUtil& testFramework,
                       bool skipASAlert = false)
   {
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "Default Constructor", __FILE__,
                                    __LINE__);
      int badCount = 0;
      ...
      testStream << "Check if subframeStore is filled with 0s. " << badCount <<
         " of the elements are incorrect.";
      testMesg = testStream.str();
      testStream.str("");
      ...
      testStream << "Check if haveSubframe[] is set to false. " << badCount <<
         " of the elements are incorrect.";
      testMesg = testStream.str();
      testStream.str("");
      testFramework.assert(badCount == 0, testMesg, __LINE__);
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "addSubframe", __FILE__, __LINE__);
      ...
      testFramework.assert(dataStore.addSubframe(subframe1P, 1025, 6, 1), testMesg,
                           __LINE__); //Week: 1025, PRN: 6, tracker:1

      subframe1Check(dataStore, testFramework);

      testMesg = "Subframe was not added successfully";
      testFramework.assert(dataStore.addSubframe(subframe2P, 1025, 6, 1), testMesg,
                           __LINE__); //Week: 1025, PRN: 6, tracker:1

      subframe2Check(dataStore, testFramework);

      testMesg = "Subframe was not added successfully";
      testFramework.assert(dataStore.addSubframe(subframe3P, 1025, 6, 1), testMesg,
                           __LINE__); //Week: 1025, PRN: 6, tracker:1
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "addSubframeNoParity", __FILE__,
                                    __LINE__);
      ...
      testMesg = "Subframe was not added successfully";
      testFramework.assert(dataStore.addSubframeNoParity(subframe1, 1025, 6, 1),
                           testMesg, __LINE__); //Week: 1025, PRN: 6, tracker:1
      ...
      testMesg = "Subframe was not added successfully";
      testFramework.assert(dataStore.addSubframeNoParity(subframe2, 1025, 6, 1),
                           testMesg, __LINE__); //Week: 1025, PRN: 6, tracker:1
      ...
      testMesg = "Subframe was not added successfully";
      testFramework.assert(dataStore.addSubframeNoParity(subframe3, 1025, 6, 1),
                           testMesg, __LINE__); //Week: 1025, PRN: 6, tracker:1
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "setSF1", __FILE__, __LINE__);
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "setSF2", __FILE__, __LINE__);
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "setSF3", __FILE__, __LINE__);
      ...
      dataStore.setSF3(0,     409914.0,   0,
                       ...
                       -.242143869400*pow(b10,-7),   .10479939309884491,     -.596046447754*pow(b10,
                                                                                                -7),
                          //I0              crc      W (aka omega)
      ...
      testStream <<
         "Complete, valid ephemeris was incorrectly considered incomplete.";
      testMesg = testStream.str();
      testStream.str("");
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "Get Methods", __FILE__, __LINE__);
      ...
      testFramework.assert(std::abs(dataStore.getHOWTime(1) - 409902) < eps,
                           testMesg, __LINE__);
      ...
      testFramework.assert(std::abs(dataStore.getAf0() - -.839701388031*pow(b10,
                                                                            -3)) < eps, testMesg, __LINE__);

      testMesg = "The getAf1 method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getAf1() - -.165982783074*pow(b10,
                                                                            -10)) < eps, testMesg, __LINE__);
...
//    testFramework.assert(dataStore.getAODO() == 0/*VALUE NOT SET BY SETSF3*/, testMesg, __LINE__);
      ...
      testFramework.assert(std::abs(dataStore.getCus() - .652112066746*pow(b10,
                                                                           -5)) < eps, testMesg, __LINE__);

      testMesg = "The getCrs method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getCrs() - 93.40625) < eps, testMesg,
                           __LINE__);

      testMesg = "The getCuc method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getCuc() - .484101474285*pow(b10,
                                                                           -5)) < eps, testMesg, __LINE__);
      ...
      testFramework.assert(std::abs(dataStore.getM0() - 0.162092304801) < eps/*?*/,
                           testMesg, __LINE__);

      testMesg = "The getDn method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getDn() - .11604054784*pow(b10,
                                                                         -8)) < eps, testMesg, __LINE__);

      testMesg = "The getEcc method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getEcc() - .626740418375*pow(b10,
                                                                           -2)) < eps, testMesg, __LINE__);

      testMesg = "The getAhalf method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getAhalf() - .515365489006*pow(b10,
                                                                             4)) < eps, testMesg, __LINE__);

      testMesg = "The getA method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getA() - pow(.515365489006*pow(b10,4),
                                                           2)) < eps, testMesg, __LINE__);

      testMesg = "The getCis method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getCis() - -.596046447754*pow(b10,
                                                                            -7)) < eps, testMesg, __LINE__);

      testMesg = "The getCrc method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getCrc() - 326.59375) < eps, testMesg,
                           __LINE__);

      testMesg = "The getCic method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getCic() - -.242143869400*pow(b10,
                                                                            -7)) < eps, testMesg, __LINE__);

      testMesg = "The getOmega0 method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getOmega0() - 0.10479939309884491) <
                           eps, testMesg, __LINE__);

      testMesg = "The getI0 method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getI0() - 0.3550481409757088) < eps,
                           testMesg, __LINE__);

      testMesg = "The getW method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getW() - 0.6587700862443613) < eps,
                           testMesg, __LINE__);

      testMesg = "The getOmegaDot method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getOmegaDot() -
                                    -2.0318111637599545*pow(b10,-9)) < eps, testMesg, __LINE__);

      testMesg = "The getIDot method didn't function correctly";
      testFramework.assert(std::abs(dataStore.getIDot() - .307155651409*pow(b10,
                                                                            -9)) < eps, testMesg, __LINE__);
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "loadData", __FILE__, __LINE__);
   ...
   {
      gpstk::TestUtil testFramework("EngEphemeris", "addIncomplete", __FILE__, __LINE__);
      ...
      testFramework.assert(dataStore.addIncompleteSF1Thru3(subframe1P, subframe2P,
                                                           subframe3P, 444, 1025, 6, 1), testMesg, __LINE__);
